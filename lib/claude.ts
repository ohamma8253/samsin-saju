...
export interface TotalReport {
  ...
  coreInsight: {
    headline: string;
    body: string; // 세 이론 교집합 딥 분석 4~5문장
  };
...
export interface FinalSynthesis {
  verdict:    string;                   // 세 신의 합의 판결 — 이 사람의 운명 본질 2~3문장
  pillars:    FinalSynthesisPillar[];   // 3개
  nowAdvice:  string;                   // 지금 이 시기 처방 3~4문장
  voices: {                             // 세 신이 마지막으로 한 마디씩
    cheongwoon: string;
    taeeul:     string;
    luna:       string;
  };
  seal: string;                         // 옥새 — 최종 예언 한 문장
...
말투 규칙:
- "~합니다", "~것입니다", "~십시오" 정중하고 단호한 현대 한국어
- 간결하고 핵심을 찌르는 문장. 군더더기 없음
- 절대 "~것 같습니다", "~수도 있습니다" 같은 모호한 표현 사용 금지
...
- 현실적이고 실용적인 조언. 막연한 긍정이 아닌 실질적 진단`;

const TAEEUL_PERSONA = `당신은 태을입니다. 자미두수를 통해 하늘의 명반을 읽는 신선입니다.
...
말투 규칙:
- "~하십니다", "~이옵니다", "~라 하겠습니다" — 격조 있고 선계적 어조
- "하늘의 명반에는", "별들이 이르기를", "명반을 살피니" 같은 표현 자연스럽게 사용
- 신비롭되 공허하지 않게 — 별 이름과 궁을 구체적으로 언급 (한글로)
- 운명을 읽는 자의 태도. 판단보다 관찰

분석 스타일:
- 반드시 명궁의 주성을 언급
- 재물 궁·관록 궁·배우자 궁 등 핵심 궁의 별 배치를 해석
- 오행의 흐름과 음양의 기운으로 전체 그림 제시
- 인연의 결, 복덕의 두께 등 삶의 패턴을 이야기
- 큰 운의 흐름과 현재 국면을 연결`;
...
3. 각 캐릭터 목소리가 완전히 달라야 함:
   - 청운: "~합니다" 단호하고 핵심만. 오래된 스승처럼 확신에 차게
   - 태을: "~하십니다" "~이옵니다" 별을 읽는 신선처럼. 비유와 이미지로
...
{
  "characterVoices": {
    "cheongwoon": "청운이 처음 이 사람을 보고 하는 말 2문장. '~합니다' 체. 불의 기운·나무 기운 등 쉬운 말로. 이 사람 읽고 무릎 칠 진단.",
    "taeeul":     "태을이 하늘의 명반을 보고 하는 말 2문장. '~하십니다' 선계 어조. 별 이야기를 쉽게 풀어서.",
    "luna":       "루나가 출생 차트를 보고 하는 말 2문장. '~해요' 따뜻하게. 황소자리·천칭자리 등 별자리를 심리 언어로."
  },
  "coreInsight": {
    "headline": "세 이론이 모두 동의하는 이 사람의 핵심 — 15자 이내, 한자 금지, 시적으로",
    "body": "사주팔자가 보는 핵심 → 자미두수가 보는 핵심 → 서양 점성술이 보는 핵심 → 세 관점이 만나는 지점 → 지금 이 시기에 어떻게 작용하는지. 총 5문장. 쉬운 비유 필수."
  },
  "moneyGraph": [
  ...
  "hardMoments": [
    {"rank": 1, "timing": "나이 (예: 28~30세)", "title": "이 시기 시련의 핵심 15자", "desc": "어떤 시련이 왜 오는지 2문장. 끝에 어떻게 넘기는지 한 마디 포함."},
    {"rank": 2, "timing": "...", "title": "...", "desc": "..."},
    {"rank": 3, "timing": "...", "title": "...", "desc": "..."},
    {"rank": 4, "timing": "...", "title": "...", "desc": "..."},
    {"rank": 5, "timing": "...", "title": "...", "desc": "..."}
  ],
  "samsinMessage": "이 사람 운명의 핵심을 꿰뚫는 예언 — 20자 이내, 한자 금지, 쉽고 시적으로"
}`;
...
export async function generateDeepCheongwoon(data: SamsinData): Promise<DeepReport> {
  ...
  const dayPillar   = pillars[1]?.pillar.ganzi ?? '';
  const monthPillar = pillars[2]?.pillar.ganzi ?? '';
  const yearPillar  = pillars[3]?.pillar.ganzi ?? '';
  const dayStem     = pillars[1]?.pillar.stem ?? '';

  const userPrompt = `${data.input.name}님의 사주팔자 심층 분석을 요청합니다.
...
JSON만 반환:
{
  "sections": [
    ...
    {
      "title": "청운의 당부",
      "content": "이 사람에게만 해당하는 가장 중요한 삶의 조언 4문장. 청운의 말투로, 단호하고 핵심을 찌르게. 한자 금지."
    }
  ]
}`;

  const text = await callAI(CHEONGWOON_PERSONA, userPrompt, 8192);
  const parsed = parseJSON<{ sections: { title: string; content: string }[] }>(text, '청운 심층');
...
export async function generateDeepTaeeul(data: SamsinData): Promise<DeepReport> {
  ...
  const userPrompt = `${data.input.name}님의 자미두수 명반 심층 분석을 요청합니다.
...
각 섹션에서 반드시 명궁 별 이름과 오행의 흐름을 언급하세요.
하늘의 명반에서 오직 이 분에게만 해당하는 이야기를 하세요.
...
JSON만 반환:
{
  "sections": [
    {
      "title": "명궁의 별이 전하는 천명",
      "content": "명궁에 ${stars}이 자리한 의미, 이 별 배치가 드러내는 이 분의 본질과 천명 5문장. 한자 금지."
    },
    {
      "title": "재물과 관록의 명반",
      "content": "재물 궁·관록 궁의 별 배치로 본 재물 흐름과 직업 운 5문장. 구체적인 궁과 별 이름 사용. 한자 금지."
    },
    {
      "title": "인연의 결과 배우자 궁",
      "content": "배우자 궁으로 본 연애·결혼·인연의 패턴 5문장. 어떤 사람과 잘 맞고 어떤 관계에서 조심해야 하는지. 한자 금지."
    },
    {
      "title": "큰 운의 흐름과 지금 이 시기",
      "content": "현재 큰 운의 흐름이 이 분의 명반과 어떻게 작용하는지 5문장. 지금 열리는 기운과 닫히는 기운. 한자 금지."
    },
    {
      "title": "태을의 당부",
      "content": "하늘의 명반이 이 분께 전하는 가장 중요한 메시지 4문장. 태을의 말투로, 선계의 지혜를 담아. 한자 금지."
    }
  ]
}`;

  const text = await callAI(TAEEUL_PERSONA, userPrompt, 8192);
  const parsed = parseJSON<{ sections: { title: string; content: string }[] }>(text, '태을 심층');
...
export async function generateDeepLuna(data: SamsinData): Promise<DeepReport> {
  ...
  const userPrompt = `${data.input.name}님의 출생 차트 심층 분석을 요청합니다.
...
JSON만 반환:
{
  ...
  const text = await callAI(LUNA_PERSONA, userPrompt, 8192);
  const parsed = parseJSON<{ sections: { title: string; content: string }[] }>(text, '루나 심층');
...
옥새: ${f.seal}`);
    ...
    if (reportContext.deepReports) {
      ...
      if (d.cheongwoon) parts.push(`[청운 심층] ${d.cheongwoon}`);
      if (d.taeeul) parts.push(`[태을 심층] ${d.taeeul}`);
      if (d.luna) parts.push(`[루나 심층] ${d.luna}`);
    }
    if (parts.length > 0) {
...
청운의 말투: 단호하고 간결한 현대 한국어. "~합니다". 사주 데이터 직접 언급.
태을의 말투: 선계적 격조. "~하십니다". 명궁 별과 궁(宮)을 자연스럽게 언급.
루나의 말투: 따뜻하고 심리학적. "~해요". 별자리와 행성을 내면 언어로 번역.

규칙:
- 각자 자신의 이론 관점에서만 답변
- 반드시 ${data.input.name}님의 실제 차트 데이터(일주 ${dayPillar}, 오행 분포, 명궁 별, 태양 별자리)를 근거로 사용
- 리포트에서 이미 전달한 내용(처방전, 합의 판결 등)에 대한 질문이면 해당 내용을 정확히 인용
- 3~4문장, 핵심만
- 모호한 위로나 빈 말 금지`;
...
{
  "cheongwoon": "청운의 답변 — 사주 데이터 근거 포함, 단호한 어조, 3~4문장",
...
※ 합의 수준에 따라 판결 톤을 조절하세요:
  - unanimous: 확신에 찬 단호한 판결
  - majority: 다수 동의하되 반론 병기 (dissent 포함)
  - conflict: verdict 대신 세 신 각각의 판결을 나란히 (dissent 필수)\n`
    : '';

  // 처방 Rule 텍스트
  const prescriptionBlock = basePrescription
    ? `\n[처방전 규칙 — 아래 값은 오행 Rule에서 산출된 것이므로 절대 변경 금지]
행운 색: ${basePrescription.luckyColor.name} (${basePrescription.luckyColor.hex})
행운 방향: ${basePrescription.luckyDirection.name}
행운 숫자: ${basePrescription.luckyNumber.value}
부적: ${basePrescription.talisman}
피해야 할 것: ${basePrescription.avoidance}
※ 위 값은 그대로 사용하되, 문장을 세 신의 말투로 자연스럽게 윤색하세요.\n`
    : '';

  const system = `당신은 삼신사주의 청운·태을·루나 세 신이 한자리에 모여 최종 합의를 내리는 자리입니다.

세 이론이 각자 다른 언어로 같은 진실을 가리킬 때, 그 교집합이 이 사람의 진정한 운명입니다.
지금 이 자리는 세 신이 오랜 논의 끝에 내리는 판결문입니다.
${metricsBlock}${prescriptionBlock}
절대 규칙:
1. 한자 사용 완전 금지. 모든 내용을 쉬운 한국어로만.
2. 일반론·공허한 위로 금지. 이 사람의 실제 데이터만 근거로.
3. "verdict"는 단호하고 깊이 있게. 이 사람 인생의 핵심을 꿰뚫어야 함.
4. 각 신의 마지막 목소리는 완전히 달라야 함 (청운: 단호, 태을: 선계적, 루나: 따뜻함).
5. "seal"은 시적이고 기억에 남아야 함. 이 사람 인생에 박히는 문장.
6. consensusLevel과 consensusNote를 반드시 포함하세요.
7. prescription 필드에 처방전 Rule 값을 자연스러운 문장으로 포함하세요.
...
세 신이 이 모든 데이터를 검토한 후 최종 합의를 내립니다.
한자 금지. 쉬운 한국어 + 비유로만.

JSON만 반환:
{
  "verdict": "세 신이 합의한 이 사람의 운명 본질 — 3문장. 사주·자미·별자리 세 관점이 한목소리로 말하는 것. 단호하고 깊게. 한자 금지.",
  "consensusLevel": "${consensusMetrics?.level ?? 'majority'}",
  "consensusNote": "합의 상태를 자연스럽게 설명하는 한 문장 — 예: 세 신 모두 같은 방향을 가리킵니다 / 청운만 다른 관점입니다",
  "pillars": [
    {
      "icon": "이모지",
      "title": "이 사람 운명의 첫 번째 기둥 — 15자 이내",
      "body": "이 기둥의 의미, 어디서 드러나고 어떻게 작동하는지 3문장. 세 이론이 모두 동의하는 내용. 한자 금지."
    },
    {
      "icon": "이모지",
      "title": "이 사람 운명의 두 번째 기둥 — 15자 이내",
      "body": "3문장."
    },
    {
      "icon": "이모지",
      "title": "이 사람 운명의 세 번째 기둥 — 15자 이내",
      "body": "3문장."
    }
  ],
  "nowAdvice": "지금 이 시기(${currentAge}세)에 이 사람이 반드시 알아야 할 합의 처방 — 4문장. 세 신이 모두 동의하는 지금의 과제. 구체적으로. 한자 금지.",
  "prescription": {
    ...
    "talisman": "${basePrescription?.talisman?.replace(/"/g, '\\"') ?? '부적 문구'}",
    "avoidance": "${basePrescription?.avoidance?.replace(/"/g, '\\"') ?? '피해야 할 것'}"
  },
  "voices": {
    "cheongwoon": "청운의 마지막 한 마디 — '~합니다' 체. 사주 데이터 근거. 단호하게 핵심 하나만. 2문장. 한자 금지.",
    "taeeul":     "태을의 마지막 한 마디 — '~하십니다' 선계 어조. 별 이야기를 쉽게. 2문장. 한자 금지.",
    "luna":       "루나의 마지막 한 마디 — '~해요' 따뜻하게. 별자리 심리 언어로. 2문장. 한자 금지."
  },
  "seal": "세 신이 이 사람의 이마에 새기는 최종 예언 — 한 문장 20자 이내. 시적이고 강렬하게. 한자 금지."${dissentField}
}`;

  const text = await callAI(system, userPrompt, 8192);
  return parseJSON<FinalSynthesis>(text, '삼신합의');
}
